...
def discretized_mix_logistic_loss(x,l,sum_all=True):
    ...
    nr_mix = int(ls[-1] / 4) # int(ls[-1] / 10) # here and below: unpacking the params of the mixture of logistics
    ...
    if sum_all:
        return log_probs, -tf.reduce_sum(log_sum_exp(log_probs))
    else:
        return log_probs, -tf.reduce_sum(log_sum_exp(log_probs),[1,2])
